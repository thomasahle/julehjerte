...
describe('drawHeart', () => {
	...
	it('draws each cell where the paint canvas points at it', () => {
		// The two ends of the merge: the import dialog draws a mask with
		// `drawHeart`, the paint canvas turns a pointer into a cell with
		// `toScreen`/`toMask` from `heartLayout`. They now share one layout, and
		// this is what that buys — the cell drawn under a point is the cell that
		// point paints, so a stroke lands under the finger.
		const ctx = paint(quadrantMask(1, 0));
		...
		// Away from the cell edges, so the round trip below is not decided by a
		// floating-point hair either side of a boundary.
		const inked = toScreen(layout, 0.8, 0.3);
		expect(colourAt(inked)).toBe(RED);
