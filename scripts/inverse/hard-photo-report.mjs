...
export async function writeHardPhotoReport(file) {
  ...
  const cards = rows.map(r => `<article data-status="${escape(r.status)}"><h2>${escape(r.label)} · ${escape(r.id)}</h2><p class="status">${escape(r.status)} · crop: ${escape(r.locatorStatus||r.cropStatus)}</p>
    ...
    ${r.runs.map(t => `<section><h3>${escape(t.preset)} · ${escape(t.status)}</h3><p>${escape((t.seconds??t.solverSeconds)?.toFixed(2))} s ${t.seconds==null?'solver':'total'} · ${t.direct?'Direct curve fit':`${escape(t.curves)} traced curves`}${t.independentImageError == null ? '' : ` · ${(100 * t.independentImageError).toFixed(2)}% independent image mismatch`}</p><p>${escape(t.message)}</p>
    <div class="images">${figure(`${r.id}/${t.preset}/mask.png`, 'Classified photograph before repairs')}${t.direct?'':figure(`${r.id}/${t.preset}/target.svg`, 'Prepared boundaries')}${t.independentImageError == null ? '' : figure(`${r.id}/${t.preset}/independent-weave.png`, 'Independent render of exported curves')}</div></section>`).join('')}
