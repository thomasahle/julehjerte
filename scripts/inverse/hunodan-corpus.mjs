...
await fs.mkdir(out,{recursive:true});const rows=[],hash=b=>createHash('sha256').update(b).digest('hex');
...
await fs.writeFile(`${out}/contact.jpg`,contact.toBuffer('image/jpeg',{quality:.92}));await fs.writeFile(`${out}/cases.json`,JSON.stringify({description:'All 27 paired sources. Only the photograph region is exposed to the locator and reconstructors. Published templates remain validation references.',cases:rows},null,2));console.log(out);
