...
- Raster crop borders now use a 1.5 mm band in the General preset of inward-profile consensus to reduce false slit endpoints. Set **Stabilize crop border** to zero to disable it. The original classified crop is retained for the binary-mask view and result image-error measurement. See [ROBUSTNESS.md](ROBUSTNESS.md) for the algorithm, the bell perturbation tests, 19 cropped collage cases, independent export renders and the proposed direct-optimization architecture.
