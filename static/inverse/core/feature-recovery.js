...
export async function recoverImageFeatures(input,cfg,seconds,onProgress=()=>{}){
  ...
  try{
    ...
    const solution=await solveGraph(graph,{...cfg,timeLimit:remaining},{onProgress,coefficientQuantum:1e-7,stopAfterValidated:true,coreGate:cfg.requireMaterialCore?s=>materialAudit(s,cfg):null});
