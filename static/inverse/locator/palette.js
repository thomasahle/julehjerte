...
export function makePaletteFields(image){
  ...
  const threshold=Math.max(.10,noise*3),lightNeutral=Math.max(...background)-Math.min(...background)<.10&&Math.min(...background)>.8;
  ...
  for(let i=0;i<n;i++){
    ...
    if(!transparent&&lightNeutral){const saturation=Math.max(...color)-Math.min(...color),dark=1-Math.max(...color);p*=Math.max(sigmoid((saturation-.10)/.02),sigmoid((dark-.32)/.04));}
    paper[i]=p;
  ...
  return{w,h,paper,red,white,chroma,r,g,b,mask:close(paper,w,h,2),pd:signedDistance(close(paper,w,h,1),w,h),rd:signedDistance(close(red,w,h,1),w,h),wd:signedDistance(close(white,w,h,1),w,h),palette:{method:'two-paper-colours-on-plain-background',background,transparent,backgroundNoise:noise,colors:centers,separation}};
}
