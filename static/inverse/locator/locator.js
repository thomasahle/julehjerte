...
function fitModel(f,box,lines,{iterations=550}={}){const[x0,y0,x1,y1]=box,w=x1-x0,h=y1-y0,scale=(w+h)/2,seed=[(x0+x1)/(2*scale),(y0+.094*h)/scale,.453*h/scale,.453*h/scale];
 ...
 function evaluate(z,details=false){if(z[2]<.18||z[2]>.82||z[3]<.18||z[3]>.82)return details?null:1e5;const m=model(z,lines,scale);if(!m)return details?null:1e5;const{H,q}=m;
  if(Math.abs(H[6])>.65||Math.abs(H[7])>.65||q.some(p=>p[0]<-w*.15||p[0]>f.w+w*.15||p[1]<-h*.12||p[1]>f.h+h*.12))return details?null:1e5;
  const polys=CONTOURS.map(v=>v.map(p=>project(H,p)));let opts=[];
  for(let mode=0;mode<2;mode++){// mode 0: right/top cap white; left cap red.
   const capDist=mode?[f.rd,f.wd]:[f.wd,f.rd],capProb=mode?[f.red,f.white]:[f.white,f.red];
   ...
   const clip=Math.max(8,w*.05);let loss=(1.5*trimMean(arcResiduals[0].map(x=>Math.min(x,clip)))+1.5*trimMean(arcResiduals[1].map(x=>Math.min(x,clip)))+trimMean(sideResiduals[0].map(x=>Math.min(x,clip)))+trimMean(sideResiduals[1].map(x=>Math.min(x,clip))))/5;
  ...
  return details?{...best,loss:best.loss+penalty,q,H,polys}:best.loss+penalty;
 }
 const starts=[seed,seed.map((v,i)=>v+(i===0?-.025:i===1?.025:0)),seed.map((v,i)=>v+(i===0?.025:i===1?-.025:0))];
 const fits=starts.map(s=>nelderMead(evaluate,s,[.025,.025,.025,.025],{maxIterations:iterations})).sort((a,b)=>a.f-b.f),best=evaluate(fits[0].x,true);
